...
const keywords = {
  ...
  properties (value, data) {
    const dataProps = Object.keys(data.value)
    const props = dataProps.filter((prop) => value.hasOwnProperty(prop))
    const validators = props.map((prop) => {
      ...
      return jsv.compile().then((instance) => {
        return instance.validate(data.value[prop])
      }).catch((err) => {
