...
const keywords = {
  default (value, data) {
    if (value === DEFAULT_NOW && typeof data.value === 'undefined') {
      if (this.schema.hasOwnProperty('format')) {
        switch (this.schema.format) {
          case 'date':
            ...
            break
          case 'time':
            data.value = new Date().toLocaleTimeString().replace('/.*(\\d{2}:\\d{2}:\\d{2}).*/', '$1')
            break
          case 'timestamp':
            data.value = new Date().getTime().toString()
            break
          case 'date-time':
            data.value = JSON.parse(JSON.stringify(new Date()))
            break
          default:
            this.setDefaultValue(value, data)
        }
      } else {
        this.setDefaultValue(value, data)
      }
  ...
  format (value, data) {
    switch (value) {
      case 'time':
        return re.time.test(data.value)

      case 'timestamp':
        return new Date(parseInt(data.value)).getTime() > 0
...
class GenericString extends Base {
  ...
  validateDefaultValue () {
    if (this.schema.default === DEFAULT_NOW) {
      if (this.schema.hasOwnProperty('format')) {
        return
      }
    }
    return super.validateDefaultValue()
  }
